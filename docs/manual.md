...
├── src/
│   ├── config/           ← 内容种子（站点信息/动态/链接/项目/歌单），改内容主路径是 /admin
...
> **日常编辑的主路径是 `/admin` 网页界面**（文章/动态/链接/项目/装备/歌单/井外低语/站点信息/插图音乐上传），改完即时生效，无需碰代码。
...
- **阶段 3（已完成）**：前端接线、`/admin` 管理界面（用户/留言/文章/七项设置（含井外低语）/上传，日常编辑全网页化）、`/projects` 载荷舱、关于逃逸装备、动态/收藏/留言墙（跨设备可见）完整化。
