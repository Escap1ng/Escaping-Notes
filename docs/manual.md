...
顶栏「音乐」按钮播放本地 mp3 曲目（阶段四已接通，无需外部音乐 API）。曲目清单与音频上传见 `/admin` 的「歌单」设置块（`title|artist|/uploads/文件名.mp3`），音频文件经 `/admin` 上传至 `/uploads/`；种子与离线回退见 `src/config/music.js`。
...
本站已发布 **v1.0.0 正式版**（2026-09）。以下里程碑均已交付：
...
- **内容与页面**：文章归档与阅读闭环、动态时间线、歌单（QQ 同步）、项目载荷舱、留言墙、关于、404；双语 eyebrow 与「现在」读数栏。
- **体验与性能**：双主题（深空/纸面）、自定义光标、音乐播放器、移动端适配、`prefers-reduced-motion` 降级、像素预算封顶。
