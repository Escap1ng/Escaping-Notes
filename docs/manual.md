...
> **日常编辑的主路径是 `/admin` 网页界面**（文章/动态/项目/装备/歌单/井外低语/插图音乐上传/图片管理），碎片在 `/fragments` 页面发布，改完即时生效，无需碰代码。
...
- **阶段 1（已完成）**：首屏引力井轨迹场 + 引力干扰 + 逃逸坐标导航（核心装置）。
- **阶段 2（已完成）**：后端 v2（注册/登录/三角色/文章 CRUD/meta 注入/RSS）+ 文章系统接通 + 标签过滤/搜索 + 阅读闭环。
- **阶段 3（已完成）**：前端接线、`/admin` 管理界面（用户/留言/文章/五项设置（含井外低语）/上传，日常编辑全网页化）、`/projects` 载荷舱、关于逃逸装备、动态/留言墙（跨设备可见）完整化。
