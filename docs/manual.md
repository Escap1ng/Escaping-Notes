...
> **日常编辑的主路径是 `/admin` 网页界面**（文章/动态/链接/项目/装备/歌单/站点信息/插图音乐上传），改完即时生效，无需碰代码。
...
- **阶段 3（已完成）**：前端接线、`/admin` 管理界面（用户/留言/文章/六项设置/上传，日常编辑全网页化）、`/projects` 载荷舱、关于逃逸装备、动态/收藏/留言墙（跨设备可见）完整化。
