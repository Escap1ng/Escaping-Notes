...
├── public/               ← 静态资源：favicon 等（图片/音乐建议经 /admin 上传，存服务器 /uploads/）
├── server/               ← 极简后端 api.py（零依赖：登录/发文/留言墙/上传/内容 API）
├── src/
│   ├── config/           ← 内容种子（站点信息/动态/项目/歌单），改内容主路径是 /admin
│   ├── lib/              ← API/内容/会话前端客户端（自动维护，无需手改）
│   ├── components/       ← 头部、页脚、星轨/光标装置等公共组件（`neo/` 内为 neo 组件）
│   ├── views/            ← 每个页面一个文件（含 /admin /login /register /projects）
│   ├── router/           ← 网址路由
...
> **日常编辑的主路径是 `/admin` 网页界面**（文章/动态/项目/装备/歌单/低语/插图音乐上传/图片管理），改完即时生效，无需碰代码。注：`/fragments` 碎片页已随旧版下线。
...
- **歌单 `/records`（03 导航位）**：展示你的 QQ 音乐公开歌单静态快照（曲名/艺术家/逐行跳转）。更新方法：歌单有增删后运行 `python scripts/sync_records.py`（重新生成 `src/config/records.js`），然后 `git push` 即上线。网站运行时不调用任何外部 API；Pages 镜像同样可显示。前提是歌单设为公开。
- **碎片（代码保留，暂不占导航位）**：`/fragments` 可直达。站长登录后顶部发布器写 ≤500 字、可选附图，发布即时上墙；每条可删除。接口：`GET/POST/DELETE /api/fragments`。后端缺席时显示「信号不可达」。
...
- **友情链接**：在 `src/views/AboutView.vue` 顶部的 `friends` 数组追加 `{ label, url }`（关于页 SOCIAL 下方展示）。
...
- 头部主题开关切换，选择会自动记住。深空=近纯黑夜空（默认），纸面=纸感暖白档案风；切换是设计叙事的一部分（夜拍 / 显影），不是普通明暗切换。
- 改纸面配色：`src/styles/tokens.css` 里的 `:root[data-theme='out']` 变量块（或 `src/styles/neo.css` 的 `--cold/--hot/--white`），改完保存即生效。
- 当访客系统开启“减弱视觉效果”（prefers-reduced-motion）时，全站动画自动关停，星轨装置自动降级为一次性快进的静态底片（阶段一实现），无需你手动处理。
...
- **已完成（阶段 0）**：工程骨架、路由、页面框架、内容配置、视觉基线、示例文章、本手册。
- **阶段 1（已完成）**：首屏长曝光星轨装置 + 指针引力干扰 + 星图目录导航（核心装置）。
- **阶段 2（已完成）**：后端 v2（注册/登录/三角色/文章 CRUD/meta 注入/RSS）+ 文章系统接通 + 标签过滤/搜索 + 阅读闭环。
- **阶段 3（已完成）**：前端接线、`/admin` 管理界面（用户/留言/文章/五项设置（含低语）/上传，日常编辑全网页化）、`/projects` 载荷舱、关于装备、动态/留言墙（跨设备可见）完整化。
- **修订轮（2026-09 进行中）**：图片发布增强（编辑器插图/图片管理/阅读页灯箱/上传白名单）、碎片流 `/fragments` 替换 03 收藏位（收藏页退役）、友链移至关于页、管理界面移除 SITE 编辑块。
- **阶段 4（已完成）**：入轨转场、音乐播放器、全网计数、404、路由 title、蓄能/共振微交互。
- **阶段 5（已完成）**：移动端布局（头部紧凑/表格横滚/输入防缩放）、触摸语义（滚动取消蓄能）、焦点与 reduced-motion 复核、性能小项。
- **阶段 6（已完成，v1.0 tag 待打）**：生产构建、§10 七项自审全绿、表单样式去重重构、部署核对。用户仍有修订，tag 待修订轮结束后打。
