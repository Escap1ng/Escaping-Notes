...
- 觉得**文字看不清 / 想给某个页面加半透明底板** → 先看 §3.3 暗带契约。可读性由全站**一处**负责；在新页面上各自叠遮罩正是这次要拆掉的旧账。

---

## 1. 核心概念（≤200 字）

首页是一台架在三脚架上的相机在长曝光：数百条星轨弧绕一枚偏心天极刚体旋转累积，恒星周日视运动是全场一致的时间流速。文章是定点脉动的变星，驻目绽开锥形衍射十字芒，点击即坠入正文。色彩取自夜空的两副面孔——深空为冷白/暖白/琥珀三档星轨色温，纸面为墨与 sepia 干版轨迹 + 朱砂点睛；冷暖分工不混用。滚动即曝光加深：晕影收紧、天空转快。每一页背景沉着一枚巨型幽灵汉字，是创作者的签名。
...
- **概念**：首屏 = 架在三脚架上的相机长曝光。恒星周日视运动是刚体旋转（全场一致 ω≈0.042 rad/s），星轨绕偏心天极（全站同一处：视口 62%x / 38%y）累积成同心弧。**但刚体旋转只保证几何正确，不保证好看**：可读的星空靠的是星等分层（多数暗、极少数亮）与弧长错落，见下「星等与随机性」。
- **渲染管线（Canvas 2D · 累积缓冲）**：ACC 离屏底片按**视口**尺寸分配、全站共享（见下「天与相机」），每帧不清空——① 衰减 pass：`destination-out` 填 `alpha=fade`（尾迹长度 ∝ 1/fade）；② 增量 pass：每星画本帧短弧沉积，**弧长由 `shear`（0.06~0.40rad 随机）主导，而不是 ω·dt（每帧仅 0.0007rad）**——这一条最容易记错：调 ω 改的是天空转动的快慢，改弧长要动 `shear`。`z>0.62` 的亮星（实测占 14.2%）另在拖痕前沿叠一笔 22% 长、0.72 倍线宽、`HEAD_GAIN=2.6` 倍 alpha 的短弧，使亮度自尾向头递增（实测累积比 1.3~1.6 倍）——等亮的长弧在视觉上只是"一根光棒"。主画布每帧 = 底 → `drawImage(ACC, 0, 0, W, H)`（顺带把天区坐标缩放进本机盒子）→ 当帧层（变星/流星，不累积，保持锐利）。像素预算 4.6MP 封顶自动降 DPR，相机跟随底片 DPR。
- **天与相机（`src/lib/sky.js`）**：底片像素、星群、天极、时钟、取景权放在模块级单例 `sky` 里——**星空是一份，相机是两台**。要点：
  ...
  - **取景权**：`fall` 是并行转场，离开方与进入方并存约 260ms（leave 0.26s / enter 0.44s），两台相机同时往一张底片沉积会画双份、`resize()` 还会互相清屏。故底片同一时刻只认一个主人，后挂载者接管。**但取景权只管沉积，不管重绘**：`ownsPlate()` 只包住 `update()` 里的 `accPass()` 与抽独立尾部那一段，非主人照样每帧 `draw()`。早期版本把它写进了 `tick()` 的停帧条件，结果交出底片那台在淡出的 260ms 里**冻在半帧上**——那就是切换顿挫感的来源。
...
- **交互语义**：滚动 = 时间流速（`flow = 1 + k·2.2`，`fade` 由 0.0086→0.0038，越深尾迹越长天越快）；文章 = 变星（随天刚体旋转、脉动亮度，hover 缓绽锥形衍射十字芒·沿芒长渐隐、点击坠入）；指针 = 引力时间膨胀（180px 半径内转速×2.8、提亮，轨迹局部卷曲，移开恢复）。
...
- **防饱和尾部渐隐（1.1.0）**：星轨即将转满前 20% 平滑暗化（`SETTLE_START=0.8`、`SETTLE_DEPTH=0.55`），另随机 1-2 根做柔和独立渐隐；每颗星带随机尾迹起点偏移（`shear`），使弧段截端错落、圆环连续无对齐断口；`FADE_0=0.008 / FADE_1=0.0032` 拉长尾迹，让相邻弧段衔接成连续同心圆环。
  - **2026-09-17「久了过饱和、要更透气」回收**：`SETTLE_DEPTH → 0.72`、`FADE_0 → 0.0104`、`FADE_1 → 0.0044`、`DEP → 0.0072`（稳态亮度 ≈ DEP ÷ fade，两头一起动才不会既变淡又变短）。
  - **2026-09-18 第二轮（`SETTLE_DEPTH → 0.6`、`FADE_0 → 0.0086`、`FADE_1 → 0.0038`、`DEP → 0.0084`）**：上一档收得太狠，用户改口"不明显、不好看"。现在 `DEP ÷ FADE_0 = 0.98`，**仍低于**当初被投诉过饱和的 1.06——因为这一轮补的是**对比**而不是整体提亮（见下条）。独立尾部渐隐同时加密到 **1-3 根 / 间隔 5.2~11s**（原 1-2 根 / 9~19s）：固定抽一两根久了会听出节拍。上面两组都是历史值，别照抄，以 `StarTrails.vue` 顶部常量块为准。
- **星等与随机性（2026-09-18 重做）**：量过像素才看清"不好看"是什么——旧参数跑 400 帧后底片上点亮像素的 `p50 / p90 / 最亮 = 0.165 / 0.169 / 0.169`，**层次比 1.02**，即所有亮部一样亮，一片没有层次的雾。三处改动把它拉开：
  - **星等分布**：`z` 由 `0.22 + r^1.7·0.78`（中位数 0.6）改为 `0.16 + r^2.4·0.62`，再让 7% 的星整体 `×2.1`。累积稳态 alpha ≈ `al/(al+fade)`，实测从暗到亮 **0.196 → 0.247 → 0.443 → 0.753**（p50/p90/最亮）。线宽同步从 `0.7+z·0.9` 放宽到 `0.6+z·1.2`，让亮星在尺寸上也压过暗星。
  - **半径取样**：环带原是 4 段互不重叠、中间留 3 道空隙，直方图实测在半径 **32~36%、56~58%、80~84%** 三处共 5 个全空桶——那几道空环正是"人为同心环/太规整"的来源。改为 3 段**互相重叠**的宽带（只留"中段偏密"的倾向）+ **30% 的星半径完全随机**，现在只剩天极附近一个空桶（`r<2%`，本来就该空）。
  - **弧长与闪烁**：`shear` 从 `0.04+r·0.22` 放宽到 `0.06+r^1.25·0.34`（实测 3.4°/11.6°/22.9° 的 min/中位/max），弧长差异大到一眼读得出"每根不一样长"；闪烁幅度改随星等放大（`ta = 0.08+0.13·min(1,z)`），旧的是全场固定 0.14。
  复核方式同 §10 那条教训：**页面里复刻 `accPass` 合成数学、同步跑固定帧数、`getImageData` 量底片**（rAF 冻结也能跑）。深空与纸面两套各跑到 2400 帧，`p50` 全程钉在 0.247 / 0.165、最亮在 0.68~0.83 之间不爬向 1，即覆盖面积翻 3.6 倍的过程中**没有饱和回归**。
...
- **fall 转场**：路由跳转 = 一次下坠重开 → 展开成新页。全站唯一的内容转场（快门层与它同时发生，见下）。
  **2026-09-18 把权重从"缩"挪回"化"**：进入方 `0.44s cubic-bezier(0.52, 0.04, 0.24, 1)`、`scale(0.88) blur(15px) → 正常`；离开方 `0.26s cubic-bezier(0.36, 0.1, 0.6, 0.42)`、`→ scale(0.9) blur(12px)`。两个理由：⑴ 旧曲线 `cubic-bezier(0.16, 0.84, …)` 起步斜率高达 5.25，**虚化在前约 60ms 就散光了**，剩下的 0.36s 只是在盯着一个缩放看——慢起的 S 型让模糊留到中段才化开，两页重叠的那 260ms 才真的是叠影；⑵ 旧曲线末端（`ease-in`）是"先定住再猛抽"，换成有限末端斜率。`scale` 起点 `.8→.88` 是为了削弱几何跳幅，模糊 `12→15px` 补回失去的存在感。
  关键帧里的 `filter` 作用于**整页子树**，每一帧都要重新栅格化，且离开方与进入方并行叠影时更甚——这是本站最贵的一处绘制。1.2.0 曾评估改为"只动 opacity/transform + 独立红移覆盖层"，实为可行但**叠影模糊正是"坍缩"的手感来源，最终决定保留原样**（取舍记录见 §10）。本轮时长只从 0.42s 放到 0.44s，代价 +20ms 的整页栅格化，可接受。
- **快门层 `.neo-shutter`**：底片共享之后，换页不再是"重铺一张"，语义上就只剩**一次快门开合**。`App.vue` 用一个计数器 `shot` 给这一层重新 key（首次加载不触发——setup 跑起来时 `route.path` 已是解析完的目标页）；`animationend` 后整层摘掉；减弱动效下**整层 `display: none`**——只写 `animation: none` 会让帘幕退回未变换的 `scaleY(1)`，把页面永久盖住。另外 `shot` 在 `prefers-reduced-motion` 下干脆不递增：那一层被 `display:none` 掉了，动画不跑也就永远等不到 `animationend` 来收尾摘节点。
  **形态：羽化暗场呼吸，不是闸门（2026-09-18）**。第一版是两片 `height:50%` 的**实心** `--ink-0` 条带 `scaleY(0.55)`，朝心的前沿只有一条 2px 朱砂线收边——而 `--ink-0` 就是页面底色，于是它等于**用背景色把星轨和 fall 的虚化一起糊掉**，硬边扫过全屏就是用户说的"生硬、不如之前的虚化"。现在：条带加高到 58%，实心段只占外侧 18%，一路渐隐到 78% 处完全透明（前沿不再有硬边——硬边是"擦"，渐隐才是"化"），另叠一层很淡的朱砂暖光保住"焦平面"信号；整层 `opacity: .6`，底片透过帘幕仍在场；`0.52s`、峰值 `scaleY(.86)` 落在 46%（≈0.24s），对齐 fall 中段两页都最糊的那一瞬。只动 `transform: scaleY()`（§9.5）。
  - **两片帘幕必须同时长、同曲线**：`animationend` 会从两个伪元素各冒泡一次，`App.vue` 的摘除只在第一次触发。一旦错落时长，先结束那片会把整层连同还在跑的帘幕一起摘掉（要错落得先把摘除改成计数）。
  - **别当它做到了的说法**（实测边界，不是缺陷）：⑴ 几何是条带 `height: 58%` × 峰值 `scaleY(.86)` → 画到距外缘 49.9%，但渐变在 78% 处已全透明，所以**有色部分只到 45.2%**，其中实心段仅外侧 10.4%。于是**正中约 9.6% 的高度从头到尾一点没被压暗**，两片各自贡献的是 10.4%~45.2% 与 54.8%~89.6% 两段羽化。取个点位：30% 高度处峰值压暗约 26%（渐变 0.44 × 整层 0.6）。它是"暗场呼吸"，不是"整屏快门"；真要吃掉模糊得让实心段盖到中线，那就回到了上一版生硬的老问题。⑵ 它在 `z-index: 40`，**顶栏 60 / 进度线 65 / 灯箱 80 都在它上面**，这是有意的（仪器面板与模态不该跟着眨眼），但也就谈不上"盖在正文之上"这么绝对。
- 文章页时间膨胀读数：`τ/t = √(1−1/r)`——"你在此处停留的光阴，比人间慢一拍"。

### 3.3 暗带契约：天空与文字的位置协议

**一句话**：文字底下的天空必须被压低，天空只在带外才全亮——这件事**全站只做一处**，不外包给每张卡片。

这条契约是"P1 可读性收敛"的落点，解决的是实测出来的问题（不是审美分歧）：改之前 11 个次级页里只有文章页做过可读性底板，其余各凭运气，同一套文字色在实际合成底上的对比度是——

| 文字底下的实际情形 | text-0 | text-1 |
| --- | --- | --- |
| 裸星轨（Projects / Records / Updates / About 等） | **1.7:1** | 1.1~1.8:1 |
| 44% 卡片压在星轨上（Blog / Wall / Admin） | 4.5:1 | **1.5:1** |
| 文章页 88% 玻璃底板 | 15.7:1 | 5.4:1 ✓ |

即**同一站点内，文章页接近 AAA、列表页不到 AA 的三分之一**。这就是"阅读困难 + 体验割裂"的数值形态。

- **为什么不能靠加卡片遮罩解决**。把星轨压回可读所需的遮罩 alpha 解出来是 **81~85%**——那时候卡片等于不透明，天空白画，"长曝光"这个核心装置在列表页就名存实亡了。顺带实测到另一件事：卡片填充相对天空底**只有 1.04~1.14:1 的表面比**（肉眼几乎不可辨），卡片的边界感一直来自 `--card-brd` 那根发丝线；原先 44%→72% 的 hover 填充跳变更是只有 1.05:1，等于没有。所以 `--card-*` 重定标为「填充只管分组、悬停强调交给边线与 `.neo-spike`」（52% / 62%，是**实测区间内的口味值**，不是算出来的）。
- **三个令牌的分工**：几何在 `neo.css` §1 共享块（`--sky-band: min(1120px, 100%)`、`--sky-feather: calc((100% - var(--sky-band)) / 2)`），**压低系数分主题**在 §2/§3 各定一档（`--sky-k`）。带宽对齐 `.neo-shell` 的 `max-width: 1120px`，保证正文整块落在带内；羽化段自适应地吃掉带外的全部余量，因此不存在"内容比带宽、卡片边缘落在羽化段上过亮"的窗口。
- **两套主题的 k 是怎么解出来的**。取**最差那一档星轨色**（深空是琥珀 tier2 `#ff977d`，纸面是 tier1 墨迹 `#51240e`）在**峰值 alpha 0.753** 下合成出文字底，要求最小的次要文字 `text-1` 也过 AA 4.5：深空解得 k ≤ 0.2 → 取 **0.15**（实测 5.48:1，留 22% 余量）；纸面解得 k ≤ 0.06 → 取 **0.06**（4.65:1；k=0.1 只有 4.40，不达标）。**按峰值而不是中位数（0.247）算是必须的**：星轨在动，同一枚文字像素的底随时间明暗波动，按中位数设计等于放任它闪烁出界。纸面比深空严 2.5 倍，因为那里压的是接近纯黑的 `text-0` 与中性 `text-1`。
- **实现落点**：`StarTrails.vue` 的 scoped `.st-fixed`（次级页那块 `position: fixed` 画布）上一条 `mask-image` 横向渐变，`mask` 走 alpha 模式，故 `rgba(0,0,0,var(--sky-k))` 的 alpha 就是"天空保留多少"。选画布而不是选页面容器有两个硬理由：⑴ 一块画布服务全部 11 个次级页（isSub = path 不等于首页），改一处即全站一致；⑵ 画布在 `main` **之外**，不会像文章页进度线那样被 `fall` 转场期间页根的 `transform` 抓去当包含块。不支持 `mask-image` 的浏览器等于回到改前状态（可读性不达标但不会画错），属优雅降级。首页那台 `.st-abs` **不进这里**——它是主视觉，且它的文字已有 `--scrim` 底衬。
- **固有取舍，别当缺陷报**：视口 ≤1120px 时羽化归零，即**窄屏上一次级页的天空被整片压低**（纸面主题下几乎看不见）。手机屏上没有横向余量可留给天空，这是几何事实；大气感由首页那台不加遮罩的相机、以及宽屏两侧的侧廊承担。若日后要在窄屏找回天空，方向是给它一条**纵向**出口（顶部条带），而不是调大 `--sky-k`——那等于把可读性换回去。
- 卡片/玻璃/暗带三者的关系收敛为一句话：**暗带管"读得清"，卡片管"是一组"，文章页的 `.neo-glass` 管"长时间阅读时的隔离"**。前两者不再是同一件事。
...
| 令牌 | 值 | 说明 |
| --- | --- | --- |
| `--card-bg` | `color-mix(in srgb, var(--ink-1) 52%, transparent)` | 卡片底色（文章卡 / 留言卡 / 图片格 / 后台共用）。**只作分组，不承担可读性**——那是 §3.3 暗带的活；实测填充相对带内天空底只有 1.04~1.14:1 表面比，卡片边界感实际来自 `--card-brd` |
| `--card-bg-hover` | `… 62% …` | hover 时的轻微做实。**悬停强调由 `--card-brd-hover` 与 `.neo-spike` 承担**：旧值 44%→72% 的填充跳幅实测只有 1.05:1，等于看不见 |
...
- **快门层 `.neo-shutter`**：`neo.css` §8.5，两片**羽化**帘幕只动 `transform: scaleY()`、整层 `opacity: .6`；随每次路由变更重放。它**盖不住画面中间**、也不盖顶栏与灯箱——实测边界见 §3.2，别按"整屏快门"理解。
...
文档里最贵的不是长，是**说谎**。本项目已经出现过一次：`api.py` 长期指向一个根本不存在的
`docs/design.md`。所以引用与断言要当代码看待：

- **跨文件引用一律写明文件名**。例：`docs/design-neo.md` §10、`neo.css` §8.5 是合法写法；只在括号里留一个节号（不写文件名）即视为歧义——
  本仓库的 § 记号是**重载**的（文档章节与代码文件的分节横幅共用同一记号），裸写必然歧义。
...
- **README 那四张图也算文档**。两张底片 PNG 由 `npm run art:build` 从 `StarTrails.vue` / `sky.js` /
  ...
  出处与「想换真截图怎么做」写在 `docs/assets/readme/README.md`。
...
⑧ **2026-09-18 第二轮反馈：转场柔化 + 星轨重做**。用户反馈两句："点击标签页的切换动效很生硬，需要更柔和，不如之前的虚化效果"、"星轨设计不好看，更加明显、更加随机一点"。
- **生硬的元凶是我自己二阶段加的快门层，而且它间接推翻了 1.2.0 的决定**。1.2.0 那次是**评估过之后保留** `fall` 的整页模糊（"叠影模糊正是坍缩的手感来源"），本轮没有任何一条改动碰过 `fall` 的关键帧——但二阶段新加的 `.neo-shutter` 是两片**实心** `--ink-0` 条带，而 `--ink-0` 就是页面底色（neo 皮肤深空 `#020204` / 纸面 `#f5f2ea`，实测计算值确认）。于是换页时有一整条不透明的背景色带横扫过屏，把星轨和正在化开的模糊一起糊掉：**保留模糊的决定在形式上还写着，在观感上已经被抹掉了**。教训：往转场链路上叠新图层时，"我没改动原动画"不构成免责——一个盖在上面的实色层就足以让下面所有的缓动失去意义。改法见 §3.2（羽化 + 整层 6 成不透明 + 峰值对齐 fall 中段）。
- **上一轮的"更透气"收过了头**。`DEP ÷ FADE_0` 从 1.1.0 的 1.06 一步收到 0.69，用户紧接着就要"更明显"。这次没有回到 1.06，而是落在 0.98 并把力气放在**对比**上。之所以敢这么定，是因为量出了"不好看"的数字化特征：旧参数跑 400 帧后，底片上点亮像素的 `p50 / p90 / 最亮 = 0.165 / 0.169 / 0.169`，**层次比 1.02**——所有亮部一样亮，是一片雾而不是满天星。新参数同一测法为 `0.196 / 0.247 / 0.753`。环带空洞同理，是直方图上 5 个**完全为空**的 2% 半径桶（32~36% / 56~58% / 80~84%）而非主观判断。详细取值与理由见 §3.1「星等与随机性」。
- **方法上真正兑现了 ⑦ 那条教训**。⑦ 记下"画布类改动没有量过像素输出就不算已验证"，本轮照做：在页面里**复刻 `accPass` 的合成数学**（衰减 pass + 每星短弧 + 头部亮段），**同步**跑固定帧数后 `getImageData` 量底片——同步循环绕开了内置浏览器 rAF 冻结，这一条路子是通的。深空/纸面两套各跑到 2400 帧（≈40s，天空转过约 96°），`p50` 全程钉在 0.247 / 0.165、峰值在 0.68~0.83 之间不爬向 1，说明覆盖面积翻 3.6 倍的过程中没有饱和回归（**注意**：全底片均值一直在涨，那是弧还在往未画过的角度铺，不是变亮；判饱和要看 p50 与峰值）。CSS 侧走 CSSOM + `getComputedStyle(el,'::before')`，确认两层渐变与 `color-mix` 都求出了真色、两片帘幕同时长同曲线、`neo-blind` 有 46% 峰值——因为快门失效的方式是**静默不可见**，构建通过证明不了它活着。
- **仍未验证的部分（不要误读成已验收）**：整轮**没有任何一项经过目视**。上面量的是"底片上有多少alpha、规则在不在"，读不出好不好看。具体待用户确认的三件事：⑴ 切换是否真的变柔、快门还在不在"能察觉但不抢眼"的位置（若仍嫌抢眼，**整层可单独删**：`App.vue` 的 `shot` + `neo.css` 的 §8.5 与 `neo.css` §11，三处，与 `fall` 完全解耦）；⑵ 星轨的亮度/长度是否到位，纸面主题（fade×1.4、dep×1.5）比深空更冲，需单独看一眼；⑶ 亮星头部亮段在小屏（110 星）上够不够。所有口味值集中在 `StarTrails.vue` 顶部常量块。

⑨ **2026-09-18 P1：暗带契约 + 卡片重定标**。起因是用户提出「整体显示风格没有做很好的统一，存在阅读困难、效果不佳、体验割裂，要一套整体的解决方案」。
- **普查先推翻了我的预设**。逐文件量下来，**令牌化程度其实已经很高**——15 个视图/组件（12 视图 + 3 个壳层组件）里，非 `clamp()` 的 `font-size` 全部走 `--fs-*`、间距全部走 `--space-*`、颜色零裸值；只有 6 处纯字面量的 `clamp()` 展示字号游离在刻度之外（另 2 处 `clamp()` 是用 `--fs-*` 搭的，属合规）——令牌刻度里本就没有"流体字号"这一档。所以病灶**不在数值层**（1.3.0 那次收敛是真做到了的），而在**结构层**，三条：⑴ 可读性底板没有契约（11 个次级页只有文章页做过）；⑵ 行长没有契约（`--measure` 只有 3 页在用，列表页正文铺满 1056px ≈ 75 字/行，是文章栏的 2.2 倍）；⑶ 骨架二套（`.neo-shell` 1120/`--space-5` 与 `.page` 1120/`--space-4`，登录/注册/后台走后者）。**本轮只落地 ⑴**，⑵⑶ 是 P2/P3。
- **决策依据全部是量出来的**，不是审美判断：改前对比度见 §3.3 表；"卡片填充一直不承重"是实测出 1.04~1.14:1 表面比才知道的；`--sky-k` 两套主题各 0.15 / 0.06 是解不等式解出来的（详见 §3.3）。这里也记录一句自我批评：上一批为了"更明显"把星轨峰值 alpha 从 0.169 抬到 0.753，**当时并不知道那会直接砸穿列表页的文字对比度**——因为那时还没有"文字底下的合成底"这个量。两个诉求（天空更亮 / 文字更清）本来是耦合的，只有量出来才看得见。
- **验证边界（不要误读成已验收）**：已验的是「CSS 被浏览器接受且几何与手算一致」——用固定宽度探针（1440 / 1200 / 1120 / 900）读回 `mask-image` 计算值，确认嵌套 `calc()` + `min(1120px, 100%)` + 百分比**未被丢弃、原式保留**，带内边与羽化宽度对上了算式；令牌确实继承到画布（`--sky-k: .15`）。**未验的有三项**：⑴ 「暗带是否真的对准内容列」——这台环境 `innerWidth` 与 `documentElement.clientWidth` 都是 0、画布因 `resize()` 提前返回而退化成 300×150，量不了；⑵ 全屏 `fixed` 画布加 mask 的**帧率代价**（低端安卓上是真实风险，兜底方案是改在 `draw()` 里对中央列填一次渐变）；⑶ mask 的 alpha 乘法语义是按规范推定的、不是我像素量出来的。均需真机确认。
- **下一期（P2）**：行长与网格统一——列表卡收进 `--measure` 家族、宽屏改 2 列。它和 P1 有依赖关系：暗带带宽目前对齐 `.neo-shell` 的 1120px，若 P2 把内容列收窄，`--sky-band` 应跟着收窄，才能把两侧的天空还给宽屏。

⑩ **待办（本轮明确不做、但已量出依据的）**：
- **流体字号缺档**。全文共 8 处 `font-size: clamp(...)`，其中 **6 处是纯字面量**、游离在 `--fs-*` 刻度之外（`NeoHomeView` / `NeoPostView` / `NeoProjectsView` / `NeoRecordsView` 各 1、`HorizonHero` 2）。排版刻度是离散步进，没有"随视口连续变化"这一档，展示层只能就地写 clamp。要么在 §5.2 增设流体字号一族，要么明确"展示字号不入刻度"——**现状是没说过**，二选一定下来再写进来。
...
- **两个对拍脚本尚未接进 CI**：`check:api` 与 `check:docs` 目前只在本地跑。