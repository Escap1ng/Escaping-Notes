...
这条契约是"P1 可读性收敛"的落点，解决的是实测出来的问题（不是审美分歧）：改之前 9 个次级页里只有文章页做过可读性底板，其余各凭运气，同一套文字色在实际合成底上的对比度是——
...
- **实现落点**：`StarTrails.vue` 的 scoped `.st-fixed`（次级页那块 `position: fixed` 画布）上一条 `mask-image` 横向渐变，`mask` 走 alpha 模式，故 `rgba(0,0,0,var(--sky-k))` 的 alpha 就是"天空保留多少"。选画布而不是选页面容器有两个硬理由：⑴ 一块画布服务全部 9 个次级页，改一处即全站一致；⑵ 画布在 `main` **之外**，不会像文章页进度线那样被 `fall` 转场期间页根的 `transform` 抓去当包含块。不支持 `mask-image` 的浏览器等于回到改前状态（可读性不达标但不会画错），属优雅降级。首页那台 `.st-abs` **不进这里**——它是主视觉，且它的文字已有 `--scrim` 底衬。
...
- **仍未验证的部分（不要误读成已验收）**：整轮**没有任何一项经过目视**。上面量的是"底片上有多少alpha、规则在不在"，读不出好不好看。具体待用户确认的三件事：⑴ 切换是否真的变柔、快门还在不在"能察觉但不抢眼"的位置（若仍嫌抢眼，**整层可单独删**：`App.vue` 的 `shot` + `neo.css` §8.5 + §11 那一条，三处，与 `fall` 完全解耦）；⑵ 星轨的亮度/长度是否到位，纸面主题（fade×1.4、dep×1.5）比深空更冲，需单独看一眼；⑶ 亮星头部亮段在小屏（110 星）上够不够。所有口味值集中在 `StarTrails.vue` 顶部常量块。

⑨ **2026-09-18 P1：暗带契约 + 卡片重定标**。起因是用户提出「整体显示风格没有做很好的统一，存在阅读困难、效果不佳、体验割裂，要一套整体的解决方案」。
- **普查先推翻了我的预设**。逐文件量下来，**令牌化程度其实已经很高**——13 个视图/组件里没有一处裸字号（除 4 处刻意的一次性展示值）、间距全部走 `--space-*`、颜色零裸值。所以病灶**不在数值层**（1.3.0 那次收敛是真做到了的），而在**结构层**，三条：⑴ 可读性底板没有契约（9 个次级页只有文章页做过）；⑵ 行长没有契约（`--measure` 只有 3 页在用，列表页正文铺满 1056px ≈ 75 字/行，是文章栏的 2.2 倍）；⑶ 骨架二套（`.neo-shell` 1120/`--space-5` 与 `.page` 1120/`--space-4`，登录/注册/后台走后者）。**本轮只落地 ⑴**，⑵⑶ 是 P2/P3。
